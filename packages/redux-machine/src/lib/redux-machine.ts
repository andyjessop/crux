...
export function machine<T extends Config<D>, D extends DefaultData>(
  name: string,
  config: T,
  data: D
) {
  // Flatten config to get actions with key, e.g. 'off/switchOn': (state, payload) => state
  const actionsConfig = Object.entries(config).reduce((acc, [stateKey, stateVal]) => {
  ...
  const { reducer } = createSlice(name, data, actionsConfig);

  // Create API
  const actionKeys = [
    ...new Set(Object.values(config).flatMap((state) => Object.keys(state))),
  ] as Array<keyof Nested<T>>;

  const actionMethods = actionKeys.reduce((acc, actionKey) => {
...
type Params<T> = {
  [K in keyof Nested<T>]: Parameters<
