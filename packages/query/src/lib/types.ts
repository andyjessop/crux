...
export type Resource<T extends ResourceConfig> = {
  ...
  onFetch: <K extends 'get' | keyof Mutations<T>>(
    method: K,
    callback: (
      { state }: { state: State<Data<T>, Err> },
      ...params: T extends keyof Mutations<T> ? MutationParams<T, K> : QueryParams<T>
    ) => void
  ) => void;
  ...
  onSuccess: <K extends 'get' | keyof Mutations<T>>(
    ...
    callback: (
      { state }: { state: State<Data<T>, Err> },
      data: T extends keyof Mutations<T> ? FinalReturnType<T['mutations'][K]['query']> : Data<T>
    ) => void
  ) => void;
...
export type Endpoint<Data, Err> = {
  ...
  mutations: Record<
    string,
    ((...params: any[]) => (data: Data) => any) | ((...params: any[]) => any)
  >;
...
type Data<T extends ResourceConfig> = T['query'] extends
  | ((...params: any[]) => (data: any) => Promise<infer R>)
  | ((...params: any[]) => Promise<infer R>)
  ? R
  : any;
type Err = any;
