...
export type Route<T extends Record<string, string> = any> = {
  name: keyof T & string;
  hash?: string;
...
export function createRouterSlice<T extends Record<string, string>>(config: T) {
  const initialState = {
    route: getRouteFromUrl(config, window.location.href),
  };

  const slice = createSlice('router', initialState, {
    navigateSuccess: (state: RouterState, route: Route<T>) =>
      merge(state, {
        route,
      }),
  });
